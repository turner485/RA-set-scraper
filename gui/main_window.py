...
class ROMCollectorGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RetroAchievements Game Collector v1.00.6")
    ...
    def start_collection(self, num_roms, selected_consoles):
        ...
        if not self.get_api_key().strip():
            QMessageBox.warning(self, "Missing API Key", 
                               "Please enter your RetroAchievements API key in the Settings tab.")
            return False
        
        if not self.get_download_path().strip():
            QMessageBox.warning(self, "Missing Download Path", 
